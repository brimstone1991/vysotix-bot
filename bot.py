...
BOSS_POOL = [
    {"phases": ["🐉 Дракон Лени", "🔥 Пылающий Дракон", "💀 Дракон Тьмы"], "weak_attr": "wil", "hp": 300, "reward": "🐉 Клык Дракона", "reward_desc": "Редкий аксессуар", "flavor": "Пожирает мотивацию."},
    {"phases": ["👾 Туманный Великан", "⚡ Великан Бури", "🌑 Великан Хаоса"], "weak_attr": "hp", "hp": 400, "reward": "🌿 Амулет здоровья", "reward_desc": "Даёт здоровье", "flavor": "Нарушает режим сна."},
    {"phases": ["🍔 Король Фастфуда", "🌶️ Огненный Король", "☠️ Король Яда"], "weak_attr": "hp", "hp": 350, "reward": "🥗 Щит Питания", "reward_desc": "Защищает от соблазнов", "flavor": "Отравляет привычки питания."},
    {"phases": ["📱 Повелитель Экранов", "🌀 Вихрь Отвлечений", "🕳️ Чёрная Дыра"], "weak_attr": "wil", "hp": 380, "reward": "🎯 Кольцо фокуса", "reward_desc": "Помогает не отвлекаться", "flavor": "Похищает время."},
    {"phases": ["📚 Страж Невежества", "🌫️ Туман Забвения", "🧟 Пожиратель Знаний"], "weak_attr": "int", "hp": 420, "reward": "📖 Tome мудреца", "reward_desc": "Артефакт знаний", "flavor": "Блокирует развитие."}
]
...
def new_user(name, class_key, role="child"):
    return {
        "name": name, "class": class_key, "role": role,
        "level": 1, "xp": 0,
        "attrs": {"str": 0, "int": 0, "hp": 0, "agi": 0, "wil": 0},
        "streak": 0, "last_done_date": "",
        "tasks": [], "assigned_tasks": [], "gear": [],
        "squad_id": None,
    }
...
def boss_card(boss, squad_id, users_data, squad_members):
    ...
    for mid, dmg in top[:3]:
        ...
        if m:
            top_text += f"  • {m['name']}: {dmg} урона\n"
    phase_stars = "⭐" * (phase_idx + 1)
    return f"{phase_name} {phase_stars}\n\n{hp_bar}\nHP: {boss['hp']} / {boss['hp_max']}\n\n{boss['flavor']}\n\n⚡ Слабость: {weak_name} — +50% урон!\n⏳ До конца рейда: {max(0, days_left)} дн.\n\n*Урон отряда:*\n{top_text if top_text else '  пока нет урона'}"

async def apply_boss_damage(uid, attr_key, user, users, squad_id):
    if not squad_id:
        return ""
...
async def show_menu(target, uid, edit=False):
    ...
    text = f"{CLASSES[user['class']]} *{user['name']}* {role_emoji} · Ур.{user['level']} · 🔥{user['streak']}\n✅ Сегодня: {done}/{total}"
    if pending:
...
async def handle_text(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if step == "squad_name":
        ...
        await update.message.reply_text(
            f"🏰 *Отряд «{text}» создан!*\nКод: `{squad_id}`\n\n"
            f"👨‍👦 Ты – Родитель. Отправь этот код ребёнку!\n\n/menu",
            parse_mode="Markdown"
        )
        return
    ...
    if ctx.user_data.get("awaiting_squad_code"):
        ...
        if uid in users:
            ...
            save_users(users)
            save_squads(squads)
            await update.message.reply_text(f"✅ Вступил в *{squads[squad_code]['name']}*! Твоя роль: Ребёнок 🧒\n/menu", parse_mode="Markdown")
        else:
...
async def show_tasks(query, uid):
    ...
    for t in own_tasks:
        ...
        kb.append([InlineKeyboardButton(f"{status} {t['name']} {bonus}", callback_data=f"done_{t['id']}")])
    ...
    if assigned_tasks:
        ...
        for t in assigned_tasks:
            ...
            kb.append([InlineKeyboardButton(f"{status} {t['name']} {bonus} (от {parent})", callback_data=f"adone_{t['id']}")])
    ...
    if weak_attr:
        text += f"\n\n⚡ *Слабость босса:* {ATTRS.get(weak_attr, '⚡')}"
...
async def show_squad_menu(query, uid, squad_id):
    ...
    for mid in squad.get("members", []):
        ...
        members_text += f"{CLASSES[m['class']]} *{m['name']}* {role_emoji} — Ур.{m['level']}"
        if pending:
    ...
    if creator_id and creator_id in users:
        members_text += f"\n👑 *Создатель:* {users[creator_id]['name']}\n"
...
async def handle_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # Профиль
    if data == "profile":
        await query.edit_message_text(char_card(user), reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("◀️ Назад", callback_data="menu")]]), parse_mode="Markdown")
        return
    ...
    if data.startswith("done_"):
        ...
        boss_msg = await apply_boss_damage(uid, task["attr"], user, users, user.get("squad_id"))
        save_users(users)
    ...
    if data.startswith("adone_"):
        ...
        if task.get("assigned_by"):
            try:
                await update.get_bot().send_message(chat_id=int(task["assigned_by"]), text=f"👨‍👦 *{user['name']}* выполнил задание!\n✅ {task['name']}", parse_mode="Markdown")
            except:
                pass
    ...
    # ДАТЬ ЗАДАНИЕ (только для родителей)
    ...
    if data.startswith("assign_attr_"):
        ...
        task = {"id": str(uuid.uuid4())[:8], "name": task_name, "attr": attr_key, "xp_gain": 30, "attr_gain": 2, "done_date": "", "assigned_by": uid, "assigned_by_name": user["name"]}
        ...
        try:
            await update.get_bot().send_message(chat_id=int(target_uid), text=f"👨‍👦 *{user['name']}* дал задание!\n\n📋 *{task_name}*\nВыполни в /menu → Задания", parse_mode="Markdown")
        except:
            pass
    ...
    if data == "boss_next":
        ...
        if squad_id:
            ...
            if squad_id in bosses:
                del bosses[squad_id]
                save_bosses(bosses)
            await query.edit_message_text("🔄 Новый босс появится в рейде!")
        return
